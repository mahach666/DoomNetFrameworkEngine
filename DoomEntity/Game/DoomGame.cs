...
using DoomNetFrameworkEngine.DoomEntity.Intermission;
...
namespace DoomNetFrameworkEngine.DoomEntity.Game
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill)DoomNetFrameworkEngine.DoomEntity.MathUtils.NetFunc.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = DoomNetFrameworkEngine.DoomEntity.MathUtils.NetFunc.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = DoomNetFrameworkEngine.DoomEntity.MathUtils.NetFunc.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = DoomNetFrameworkEngine.DoomEntity.MathUtils.NetFunc.Clamp(map, 1, 9);
			}
